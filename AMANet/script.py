...
def train(args):
    ...
    # folder_path = []
    # for path in os.listdir(config.dataset_path):
    #     folder_path.append(path)
    folder_path = 'shanghaitech/training/frames'
    train_set = ImageFolder(config.dataset_path, folder_path,config.clips_length , config.frame_interval , config.img_size, config.rgb_tags)
...
def eval_model(dataset_path,dataset_name, log_dir ,model_type):
    config.log_path = os.path.join('/root/autodl-tmp/', 'log' , dataset_name ) 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='shanghaitech', choices=['ped2', 'avenue', 'shanghaitech'], help="selected datasets")
    parser.add_argument('--dataset_path', type=str, default='/root/autodl-tmp/shanghaitech', help="datasets root path ")
    ...
    parser.add_argument('--eval', type=int, default=1, help="evaluation")
